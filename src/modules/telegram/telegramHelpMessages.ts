...
export function devBotQuickSheet(cfg: RuntimeConfig): string {
  return [
    ...
    "`/ai …` = HOHOBOT (Ollama + cloud) — `hobot serve` sur l’iMac · `/agent …` = agent Claude + outils",
...
export function devBotHelpFull(cfg: RuntimeConfig): string {
  return [
    ...
    "",
    "▸ HOHOBOT (IA hybride sur l’iMac)",
    "`/ai …` — routeur local + cloud (`@cloud`, `@fast` dans le texte). Serveur : `hobot serve` · variables `HOHOBOT_BASE_URL`, `HOHOBOT_API_TOKEN` si auth.",
    "`/agent …` — boucle Claude + outils workspace (ANTHROPIC_API_KEY sur la machine Python).",
    "",
    "▸ Autre",
