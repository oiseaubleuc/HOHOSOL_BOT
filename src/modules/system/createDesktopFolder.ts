...
export async function createDesktopFolder(ws: WorkspaceManager, rawFolderName: string): Promise<CreateDesktopFolderResult> {
  ...
  try {
    ...
    target = resolveDesktopFolderPath(name);

    if (await pathExists(target)) {
      const st = await fs.stat(target);
      if (st.isDirectory()) {
        await appendSystemLog(ws, `${ts}\tcreate-folder\t${target}\texists\n`);
        return { ok: true, status: "exists", target, message: "Folder already exists ⚠️" };
      }
      await appendSystemLog(ws, `${ts}\tcreate-folder\t${target}\tpath-not-directory\n`);
      return {
        ok: false,
        target,
    ...
    await appendSystemLog(ws, `${ts}\tcreate-folder\t${target}\tcreated\n`);
    return { ok: true, status: "created", target, message };
  } catch (err) {
    const m = err instanceof Error ? err.message : String(err);
    await appendSystemLog(ws, `${ts}\tcreate-folder\t${target ?? "n/a"}\terror\t${m}\n`).catch(() => undefined);
